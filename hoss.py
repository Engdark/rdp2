    ...
    try:
        ...
        if encrypted_id:
            if encrypted_id not in active_bans:
                ...
                response = requests.post('https://botv61gsh.atwebpages.com/id.php', data={'id': encrypted_id}, verify=False)
                if response.status_code == 200:
...
def ban_countdown(encrypted_id):
    try:
        ...
        if hours > 0:
            ...
            response = requests.post('https://botv61gsh.atwebpages.com/id.php', data={'id': encrypted_id, 'action': 'delete'}, verify=False)
            if response.status_code == 200:
